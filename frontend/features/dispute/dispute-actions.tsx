...
function canOpenDispute(order: Order, user?: User | null) {
  return (
    !!user &&
    (user.id === order.customer_id || user.id === order.engineer_id) &&
    ...
    return (
      <div className="space-y-4">
        <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4">
          ...
          {dispute.resolution ? <p className="mt-2 text-sm text-rose-700">Решение: {dispute.resolution}</p> : null}
        </div>
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3">
      <div className="space-y-2">
        ...
        <Textarea id="dispute-reason" className="rounded-2xl border-slate-300" placeholder="Кратко опишите проблему: сроки, качество, несоответствие договоренности." {...form.register("reason")} />
