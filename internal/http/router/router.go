...
func New(services *service.Registry) http.Handler {
	...
	profileHandler := handler.NewProfileHandler(services.Profile, services.Media)
	...
	deliverableHandler := handler.NewDeliverableHandler(services.Deliverable)
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		api.Group(func(secure chi.Router) {
			...
			secure.Post("/profiles/me/avatar", profileHandler.UploadAvatar)
			secure.Post("/cards", cardHandler.Create)
			secure.Put("/cards/{id}", cardHandler.Update)
			secure.Post("/cards/{id}/media/cover", mediaHandler.UploadCover)
			secure.Post("/cards/{id}/media/preview", mediaHandler.UploadPreview)
			secure.Post("/cards/{id}/media/full", mediaHandler.UploadFull)
			secure.Post("/cards/{id}/media/materials", mediaHandler.UploadMaterials)
			secure.Get("/cards/{id}/download", mediaHandler.DownloadFull)
			secure.Get("/cards/{id}/materials", mediaHandler.ListMaterials)
			secure.Get("/cards/{id}/materials/{media_id}/download", mediaHandler.DownloadMaterial)
			...
			secure.Route("/orders", func(orderRoutes chi.Router) {
				...
				orderRoutes.Route("/{id}", func(order chi.Router) {
					...
					order.Get("/deliverables", deliverableHandler.List)
					order.Post("/deliverables", deliverableHandler.Upload)
					order.Get("/deliverables/{deliverable_id}/download", deliverableHandler.Download)
